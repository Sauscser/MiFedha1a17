...
const FetchSMNonCovLns = () => {
  ...
  const fetchLoanees = async () => {
    ...
    setIsLoading(true);
    try {
      await getCurrentUser(); // Ensures user is authenticated
  ...
  return <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        item
      }) => <LnerStts SMAc={item} onSyncComplete={fetchLoanees} />} keyExtractor={(item, index) => index.toString()} // Ensure unique keys
      onRefresh={fetchLoanees} refreshing={isLoading} keyboardShouldPersistTaps="handled" />
            </View>
