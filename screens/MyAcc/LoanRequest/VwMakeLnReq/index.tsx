...
import { View, Text, TextInput, FlatList, Alert, StyleSheet, TouchableOpacity, KeyboardAvoidingView, Platform } from 'react-native';
...
const FetchSMNonCovLns = props => {
  ...
  const handleSearch = text => {
    ...
    const filtered = Loanees.filter(loanee => loanee.name.toLowerCase().includes(lowerText));
  ...
  return <KeyboardAvoidingView style={{
    flex: 1
            ...
            <View style={styles.searchBar}>
                <TextInput placeholder="Pal Name" value={awsEmail} onChangeText={handleSearch} style={styles.searchInput} editable={true} />
            </View>
        ...
        item
                            ...
                            <LnerStts SMAc={item} />
                            <TouchableOpacity style={styles.copyButton} onPress={() => copyToClipboard(item.awsemail)}>
                                <Text style={styles.copyButtonText}>Copy Email</Text>
                            </TouchableOpacity>
                        </View>} keyExtractor={(item, index) => index.toString()} refreshing={loading} /> : <Text style={styles.placeholderText}>Start typing your pal name.</Text>}
        </View>
