...
const FetchSMNonLnsSnt = () => {
  ...
  useEffect(() => {
    if (!buyerFilter) {
      ...
      const filtered = allRecords.filter(item => item?.sokoname?.toLowerCase().includes(buyerFilter.toLowerCase()));
  ...
  return <View style={styles.container}>
      <View style={styles.inputBlock}>
        ...
        <TextInput placeholder="Item Name. Even partially" value={buyerFilter} onChangeText={setBuyerFilter} style={styles.input} />
        ...
        <TouchableOpacity onPress={() => setSelectMode(prev => !prev)} style={styles.selectModeBtn}>
          <Text style={styles.fetchBtnText}>
            {selectMode ? "Exit Select Mode" : "Enable Select Mode"}
          </Text>
        </TouchableOpacity>
        {selectMode && <TouchableOpacity onPress={handleMassDelete} style={styles.deleteBtn}>
            <Text style={styles.fetchBtnText}>
      ...
      item
    }) => <NonLnRec SMAc={item} selectMode={selectMode} isSelected={selectedItems.has(item.id)} toggleSelect={() => toggleSelectItem(item.id)} onDeleteComplete={fetchLoanees} />} keyExtractor={item => item.id} refreshing={loading} onRefresh={fetchLoanees} ListHeaderComponent={() => <>
